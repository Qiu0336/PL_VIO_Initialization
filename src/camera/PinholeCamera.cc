...
/**
 * \brief Lifts a point from the image plane to its projective ray and reproject to image plane
 *
 * \param p image coordinates
 * \param P image coordinates (after distortion)
 */
void
PinholeCamera::liftReprojective(const Eigen::Vector2d& p, Eigen::Vector2d& P) const
{
    double mx_d, my_d,mx2_d, mxy_d, my2_d, mx_u, my_u;
    double rho2_d, rho4_d, radDist_d, Dx_d, Dy_d, inv_denom_d;
    //double lambda;
    ...
    double resx, resy;
    resx = mParameters.fx()*mx_u + mParameters.cx();
    resy = mParameters.fy()*my_u + mParameters.cy();
    P << resx, resy;
}
