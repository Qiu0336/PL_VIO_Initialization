...
// 加载IMU数据，包括时间、位姿和IMU数据
void LoadPose(std::string filename, std::vector<IMUData>& pose)
{
...
// 保存IMU数据，包括时间、位姿和IMU数据
void save_Pose(std::string filename, std::vector<IMUData> pose)
{
    std::ofstream save_points;
    save_points.open(filename.c_str());
...
void save_Pose_asTUM(std::string filename, std::vector<IMUData> pose)
{
    std::ofstream save_points;
